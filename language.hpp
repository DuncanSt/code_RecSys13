...
public:
  topicCorpus(corpus* corp, // The corpus
              ...
              double latentReg, // Parameter regularizer used by the "standard" recommender system:0
              double lambda) : // Word regularizer used by HFT:0.1
...
//      testVotes.erase(*it);
    ...
    NW = 1 + 1 + (K + 1) * (nUsers + nBeers) + K * nWords;//K=5
    ...
    for (int i = 0; i < NW; i++)
      W[i] = 0;
    //Offset parameter:alpha= ,"peakiness" parameter:kappa=,User offset parameters:beta_user,Item offset parameters:beta_beer
    //User latent factors:gamma_user,Item latent factors:gamma_beer,Weights each word in each topic:topicWords
    ...
    *alpha /= trainVotes.size();//测试样例中评分平均值
    ...
    // Actually the model works better if we initialize none of these terms 0.1
    if (lambda > 0)
    {
      *alpha = 0;
      for (int u = 0; u < nUsers; u++)
    ...
    {
      wordTopicCounts[w] = new int[K];//K=5
      for (int k = 0; k < K; k++)
    ...
    for (std::vector<vote*>::iterator vi = trainVotes.begin(); vi != trainVotes.end(); vi++)
    {
      for (std::vector<int>::iterator it = (*vi)->words.begin(); it != (*vi)->words.end(); it++)//遍历训练数据集中的每一条评论
      {
        totalWords++;
        backgroundWords[*it]++;//计算每个词在所有评论中的权值
      }
    }
    for (int w = 0; w < nWords; w++)
      backgroundWords[w] /= totalWords;//计算每个词在所有评论中的权值比例
