                      ...
                      bool init)
{
  ...
  *beta_user = g + ind;//g+2
  ind += nUsers;
  *beta_beer = g + ind;//g+2+nUsers=g+2+24071
  ind += nBeers;//2+nUsers+nBeers=2+24071+4211

  for (int u = 0; u < nUsers; u++)
  {
    (*gamma_user)[u] = g + ind;
    ind += K;//2+24071+4211+5*24071
  }
  for (int b = 0; b < nBeers; b++)
  {
    (*gamma_beer)[b] = g + ind;
    ind += K;//2+24071+4211+5*24071+5*4211
  }
  for (int w = 0; w < nWords; w++)
  {
    (*topicWords)[w] = g + ind;
    ind += K;//2+24071+4211+5*24071+5*4211+5*25004
  }

  if (ind != NW)//NW = 1 + 1 + (K + 1) * (nUsers + nBeers) + K * nWords;//K=5
  {
...
/// Derivative(派生物、导数) of the energy function
void topicCorpus::dl(double* grad)
{
  ...
  double da = 0;
#pragma omp parallel for reduction(+:da)//主要用来对一个或多个参数条目指定一个操作符；每个线程将创建参数条目的一个私有拷贝，
                                        //在区域的结束处，将用私有拷贝的值通过指定的运行符运算，初始的参数条目被运算结果更新
  for (int u = 0; u < nUsers; u ++)
  {
    for (vector<vote*>::iterator it = trainVotesPerUser[u].begin(); it != trainVotesPerUser[u].end(); it ++)
    {
      vote* vi = *it;
      double p = prediction(vi);//Predict a particular rating given the current parameter values
...
void topicCorpus::save(char* modelPath, char* predictionPath)
{
  if (modelPath)
  {
    ...
    if (lambda > 0)
      ...
      {
        vector < pair<double, int> > bestWords;
        for (int w = 0; w < nWords; w++)//5000
          bestWords.push_back(pair<double, int> (-topicWords[w][k], w));
        sort(bestWords.begin(), bestWords.end());//从小到大的顺序
        for (int w = 0; w < nWords; w++)//5000
...
void topicCorpus::train(int emIterations, int gradIterations)
{
  ...
  for (int emi = 0; emi < emIterations; emi++)
  {
    ...
    param.epsilon = 1e-2;
    param.delta = 1e-2;
    lbfgs(NW, x, &fx, evaluate, progress, (void*) this, &param);//拟牛顿非线性最优
    printf("\nenergy after gradient step = %f\n", fx);
    lbfgs_free(x);

    if (lambda > 0)
    {
      updateTopics(true);//Update topic assignments for each word
      normalizeWordWeights();//Subtract averages from word weights
      topWords();//Print out the top words for each topic
    }

    double train, valid, test, testSte;
    validTestError(train, valid, test, testSte);//Compute the validation and test error
...
//  corpus corp("Arts.votes.gz", 0);
  topicCorpus ec(&corp, K, // K
                 latentReg, // latent topic regularizer 0
                 lambda); // lambda 0.1
  ec.train(50, 50);
