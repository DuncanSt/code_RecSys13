...
public:
  corpus(std::string voteFile, int max)
  {
    ...
    while (std::getline(in, line))
    {
      ...
      {//nw：评论字数
        ...
        if (wordCount.find(sWord) == wordCount.end())
          wordCount[sWord] = 0;
        wordCount[sWord]++;//统计语料中每个单词出现的次数
      }
      ...
      if (bCounts.find(bName) == bCounts.end())
        bCounts[bName] = 0;
      uCounts[uName]++;//map唯一标识user
      bCounts[bName]++;//map唯一标识item
    ...
    in.close();
    //nUsers = 24071, nItems = 4211, nRatings = 27980(评论数量)
    ...
    sort(whichWords.begin(), whichWords.end(), wordCountCompare);//To sort words by frequency in a corpus，升序排列
    if ((int) whichWords.size() < maxWords)
      maxWords = (int) whichWords.size();
    nWords = maxWords;//重复出现的只算一次...数据字典大于5000仍然按5000来计算.。。代码里没起作用，忽略不计
    printf("nWords:%d/n",nWords);
    ...
    while (std::getline(in2, line))
    {
      ...
      if (uCounts[uName] >= userMin)//userMin=0
      {
      ...
      if (bCounts[bName] >= beerMin)//beerMin=0
      {
